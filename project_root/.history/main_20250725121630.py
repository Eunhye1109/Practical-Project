...
def run_full_analysis(corp_name: str):
    ...
    # 2. matcher 초기화 (batch 매칭 전에 반드시 선언)
    matcher = ColumnMatcher(STANDARD_COLUMNS, manual_map=MANUAL_MAP)

    # 3. 기업 단위로 raw 컬럼 한 번만 뽑아서 batch 매칭
    all_raw_cols = sorted({col for df in data_by_year.values() for col in df.index})
    matched_map = matcher.batch_match_columns(
        targets=STANDARD_COLUMNS,
        raw_candidates=all_raw_cols,
        corp_name=corp_name
    )
