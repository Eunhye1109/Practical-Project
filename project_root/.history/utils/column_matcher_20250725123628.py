...
class ColumnMatcher:
    def __init__(self, standard_columns, manual_map=None, cache_path="column_cache.json", min_similarity=0.80):
        ...
        self.manual_map = {k: (v if isinstance(v, list) else [v])
+                           for k, v in (manual_map or {}).items()}
    ...
    def match(self, raw_column: str, candidates=None, corp_name=None, verbose=True) -> str | None:
        # 1. 수동 매핑
        # 1. 수동 매핑 (리스트 중 candidates 우선, 없으면 첫 번째)
        if raw_column in self.manual_map:
            for m in self.manual_map[raw_column]:
                if candidates and m in candidates:
                    if verbose: print(f"[🧠] manual_map: '{raw_column}' → '{m}'")
                    return m
            # candidates 에 없더라도, 일단 첫 번째 후보 리턴
            first = self.manual_map[raw_column][0]
            if verbose: print(f"[🧠] manual_map fallback: '{raw_column}' → '{first}'")
            return first
...
+                }
+                self._save_cache()
+            # (b) manual_map에도 새 매핑 추가
+            self.manual_map.setdefault(raw_column, []).append(best_match)
+            if verbose:
+                print(f"[➕] manual_map 추가: '{raw_column}' → '{best_match}'")
+            return best_match
        else:
