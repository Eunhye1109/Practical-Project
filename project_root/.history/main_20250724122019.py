...
from utils.api import get_corp_code_dict, fetch_financial_data
...
def run_full_analysis(corp_name: str):
    ...
    for year in YEARS:
        ...
        roe = calculate_roe(data, matcher)
        debt = calculate_debt_ratio(data, matcher)
        liquid = calculate_liquidity_ratio(data, matcher)
        op_margin = calculate_operating_margin(data, matcher)
        print(f"[{year}]")
        print(f"  ROE: {roe}%")
        print(f"  부채비율: {debt}%")
        print(f"  유동비율: {liquid}%")
        ...
        growth_dict = calculate_sales_growth(data_by_year)
        for year in YEARS:
