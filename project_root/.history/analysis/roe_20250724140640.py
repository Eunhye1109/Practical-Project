...
def calculate_roe(df, matcher):
    equity_col = matcher.match("자본총계")
    net_income_col = matcher.match("당기순이익(손실)")

    roe = df[net_income_col] / df[equity_col]
    return roe

def calculate_roe(df, matcher):
    equity_col = matcher.match("자본총계", df.columns)
    net_income_col = matcher.match("당기순이익(손실)", df.columns)

    if not equity_col or not net_income_col:
        return None
