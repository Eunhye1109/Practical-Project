...
# 이후 matcher를 분석 함수에 넘김
run_analysis(dataframe, matcher)
...
def run_full_analysis(corp_name: str):
    ...
    for year in YEARS:
        ...
        debt = calculate_debt_ratio(data)
        liquid = calculate_liquidity_ratio(data)
        op_margin = calculate_operating_margin(data)
        print(f"[{year}]")
        print(f"  ROE: {roe}%")
        print(f"  부채비율: {debt}%")
        print(f"  유동비율: {liquid}%")
