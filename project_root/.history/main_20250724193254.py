...
from analysis.core_metrics import extract_total_revenue, extract_net_income, extract_equity
...
def run_full_analysis(corp_name: str):
    ...
    for year, df in data_by_year.items():
        ...
        value = extract_total_revenue(df, matcher)
        if value is not None:
            print(f"  총매출: {value} 원")

        net_income = extract_n et_income(df, matcher)
